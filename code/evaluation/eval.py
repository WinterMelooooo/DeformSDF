...
def evaluate(**kwargs):
    ...
    with torch.no_grad():
        ...
        if scan_id < 24: # Blended MVS
            ...
            use_point3d = kwargs['use_point3d']
            #print(f"conf be {kwargs['conf']}")
            config = ConfigFactory.parse_file(kwargs['conf'])
            point3d_path = config.get_string('dataset.data_dir')
            point3d_path = os.path.join("../data", point3d_path, f"scan{scan_id}", "points3D.pt") if use_point3d else None
            if not point3d_path:
                bb_dict = np.load('../data/DTU_from_4D_20240709/bbs.npz')
                grid_params = bb_dict[str(scan_id)]

                mesh = plt.get_surface_by_grid(
                    grid_params=grid_params,
                    sdf=lambda x: model.implicit_network(x)[:, 0],#过了implicit network里面的一堆线性层，出来的是个标量的sdf距离
                    resolution=kwargs['resolution'],
                    level=conf.get_int('plot.level', default=0),
                    higher_res=True
                )
            else:
                points = torch.load(point3d_path)
                mesh = plt.get_surface_by_grid(
                    grid_params=None,
                    sdf=lambda x: model.implicit_network(x)[:, 0],
                    resolution=kwargs['resolution'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_point3d', default=False, action="store_true", help='If set,will use the points3D in data/DTU_NAME/scanID/ instead of contructing points!')
