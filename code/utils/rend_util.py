...
def get_camera_params(uv, pose, intrinsics):
    ...
    x_cam = uv[:, :, 0].view(batch_size, -1) #(1,num_pts)
    y_cam = uv[:, :, 1].view(batch_size, -1) #(1,num_pts)
    z_cam = depth.view(batch_size, -1) #(1,num_pts) 全1，不理解

    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics) # (1, num_pts, 4)

    # permute for batch matrix product
    pixel_points_cam = pixel_points_cam.permute(0, 2, 1) # (1, 4, num_pts)
...
def lift(x, y, z, intrinsics):
    ...
    x_lift = (x - cx.unsqueeze(-1) + cy.unsqueeze(-1)*sk.unsqueeze(-1)/fy.unsqueeze(-1) - sk.unsqueeze(-1)*y/fy.unsqueeze(-1)) / fx.unsqueeze(-1) * z # 解投影至三维的相机坐标系
